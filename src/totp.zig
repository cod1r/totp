...
fn genHOTPVal(key: []u8, text: []u8) !u64 {
    ...
    return bin_code % std.math.pow(u64, 10, 6);
}
...
pub fn main() !void {
    ...
    var i: u6 = 0;
    while (i <= 7) : (i += 1) {
        var steps: u64 = @intCast(u64, @divFloor((T - T0), time_step_seconds));
        try bytes.append(
            @intCast(u8, 
                (steps & (@intCast(u64, 255) << (8 * i))) >> (8 * i)));
    }
